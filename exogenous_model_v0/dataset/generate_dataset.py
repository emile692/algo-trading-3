...
def select_best_prediction_window(df, tp_pips, sl_pips, pips_size, candidate_windows, seed):
    ...
    features_dir = os.path.join(project_root, 'exogenous_model_v0', 'model', 'checkpoints')
...
def process_data(df_entry: pd.DataFrame, seed: int, inference: bool = False,
    ...
    df = df_entry.copy()
    logger.info(f'Calcul pour {split_type} set')
...
def save_processed_dataframe(df: pd.DataFrame, split_name: str, seed: int):
    ...
    """
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    split_dir = os.path.join(project_root, 'exogenous_model_v0', 'dataset', 'splits', f'seed_{seed}')
...
def generate_exogenous_dataset(seed):
    ...
    csv_path = os.path.join(project_root, 'exogenous_model_v0','dataset','external_source','EURUSD_H1.csv')
